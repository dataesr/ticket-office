...
const VariationItem: React.FC<VariationItemProps> = ({ variation }) => {
  ...
  const { content: remoteContent, refetch } = getFileContent(
  ...
  return (
    <>
      ...
      {commentsName &&
        ...
        !commentsName.includes(variation.structure.acronym) && (
          <Notice className="fr-mb-2w" type="warning" closeMode="disallow">
      ...
      <Row>
        <Col>
          ...
          <Text size="sm">
            ID de la structure:{" "}
            <strong>{variation.structure?.id || "Non renseigné"}</strong>
            <CopyButton
          ...
          <Row>
            <Text size="sm">
              Acronyme de la structure:{" "}
              <strong>{variation.structure?.acronym || "Non renseigné"}</strong>
              <CopyButton
                text={variation.structure.acronym}
                copiedText={copiedText}
                onCopy={copyToClipboard}
              />
            </Text>
            ...
            {remoteContent ? (
              <>
                ...
                {countRemoteCsv && (
                  <ul>
                    {!!countRemoteCsv.doi && (
                      <li className="fr-text--sm fr-mb-0">{`${countRemoteCsv.doi} DOI`}</li>
                    )}
                    {!!countRemoteCsv.hal_coll_code && (
                      <li className="fr-text--sm fr-mb-0">{`${countRemoteCsv.hal_coll_code} hal_coll_code`}</li>
                    )}
                    {!!countRemoteCsv.hal_id && (
                      <li className="fr-text--sm fr-mb-0">{`${countRemoteCsv.hal_id} hal_id`}</li>
                    )}
                    ...
                    {!!countRemoteCsv.nnt_id && (
                      <li className="fr-text--sm fr-mb-0">{`${countRemoteCsv.nnt_id} nnt_id`}</li>
                    )}
                  </ul>
                )}
              </>
