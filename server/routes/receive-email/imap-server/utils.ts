...
export function generateContributionLink(
  ...
  if (collectionName === "contacts") {
    switch (fromApplication) {
      ...
      case "tableaux":
        return `${baseUrl}/tableaux-contact?page=1&query=${referenceId}&searchInMessage=false&sort=DESC&status=choose`;
      case "bso":
  ...
  if (collectionName === "bso_local_variations_datasets") {
    return `${baseUrl}/bso-local-variations-datasets?page=1&query=${referenceId}&searchInMessage=false&sort=DESC&status=choose`;
  }
